...
const { count } = require("sneaks-api/models/Sneaker");
...
//TextScript
//Function that returns the percentage of scripts used in the string
...
function textScript(text){
    ...
    let scripts = countBy(text,char =>{
        let script = characterScript(char.codePointAt(0)); //Script of the current char
        return script ? script.name : "none"; //Return the script if it exists
    }).filter(({name})=>name!="none");

    //Find the total number of valid characters
    let total = scripts.reduce((n, {count}) => n + count, 0);

    return scripts.map(({name,count})=>{return `${Math.round(count*100/total)}% ${name}`;}).join(", ");
}

console.log(textScript('"woof","俄斯的狗俄斯的狗狗狗狗"тяв"'));